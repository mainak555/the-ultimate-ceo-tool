...
def trello_token_status(request, session_id):
    ...
    return util.json_response({"valid": valid, "token_generated_at": token_generated_at, "defaults": defaults})
...
def trello_workspaces(request, session_id):
    ...
    try:
        ...
        return util.json_error(str(e))

    return util.json_response(data)
...
def trello_lists(request, session_id):
    ...
    if not board_id:
        return util.json_error("'board' query parameter is required")
...
def trello_create_board(request, session_id):
    ...
    try:
        ...
        return util.json_error("Invalid JSON body")

    name = (body.get("name") or "").strip()
    if not name:
        return util.json_error("'name' is required")
...
def trello_create_list(request, session_id):
    ...
    if not name:
        return util.json_error("'name' is required")
    if not board_id:
        return util.json_error("'board_id' is required")
...
def trello_extract(request, session_id, discussion_id):
    ...
    try:
        ...
        return util.json_error(f"Extraction failed: {exc}", 500)

    return util.json_response({"items": items})
...
def trello_export_data(request, session_id, discussion_id):
    ...
    if request.method == "GET":
        try:
            ...
            return util.json_error(str(e))
        return util.json_response({"saved": bool(payload), "export": payload or {"cards": []}})
    ...
    if not isinstance(items, list):
        return util.json_error("'items' array is required")
    ...
    return util.json_response({"status": "ok", "export": payload})
...
def trello_push(request, session_id):
    ...
    if not list_id:
        return util.json_error("'list_id' is required")
    ...
    try:
        ...
        return util.json_error(f"Export failed: {exc}", 500)

    return util.json_response({"status": "ok", "result": result})
...
def trello_project_auth_url(request, project_id):
    ...
    return util.json_response({"url": url})
...
def trello_project_store_token(request, project_id):
    ...
    if not token:
        return util.json_error("'token' is required")
    ...
    return util.json_response({"status": "ok", "token_generated_at": result["token_generated_at"]})
...
def trello_project_token_status(request, project_id):
    ...
    return util.json_response({"valid": valid, "token_generated_at": token_generated_at})
