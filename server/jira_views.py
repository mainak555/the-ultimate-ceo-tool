...
def jira_project_verify(request, project_id, type_name):
    ...
    return json_response({"ok": True, "user": data})
...
def jira_session_metadata(request, session_id, type_name):
    ...
    if not project_key:
        return json_error("'project_key' is required")
...
def jira_extract(request, session_id, discussion_id, type_name):
    ...
    return json_response({"items": items or []})
...
def jira_export_data(request, session_id, discussion_id, type_name):
    ...
    if request.method == "GET":
        ...
        if payload is None:
            return json_response({"export": None, "saved": False})
        return json_response({"export": payload, "saved": True})
    ...
    return json_response({"export": saved, "saved": True})
...
def jira_push(request, session_id, type_name):
    ...
    try:
        ...
        return json_error(f"Push failed: {exc}", 500)
    ...
    return json_response({"result": push_result})
