/**
 * jira_software.js — Jira Software export adapter.
 *
 * Implements the ExportModalBase adapter interface.
 * Registered as provider "jira_software" in window.ProviderRegistry.
 *
...
(function () {
    ...
    _api(ctx, "GET", "/jira/" + ctx.sessionId + "/token-status/" + TYPE + "/")
      .then(function (d) {
        ...
        if (d.valid) {
          ...
          var err = d.error || "Not connected. Configure Jira Software credentials in Project Settings.";
  ...
  function _loadProjects(ctx) {
    ...
    _api(ctx, "GET", "/jira/" + ctx.sessionId + "/spaces/" + TYPE + "/")
      ...
      .catch(function (err) {
        ...
        _setStatus("Error loading projects: " + err.message);
      });
  }
  ...
  function _loadSavedExport(ctx) {
    ...
    _api(ctx, "GET", "/jira/" + ctx.sessionId + "/export/" + encodeURIComponent(ctx.discussionId) + "/" + TYPE + "/")
      .then(function (data) {
        ...
        if (_state.exported) {
          _setStatus("Already exported to Jira Software. Click Extract Items to unlock editing.");
        } else {
  ...
  function _onExtract(ctx) {
    ...
    _api(ctx, "POST", "/jira/" + ctx.sessionId + "/extract/" + encodeURIComponent(ctx.discussionId) + "/" + TYPE + "/")
      .then(function (d) {
        ...
        _state.exported = false;
        _renderIssues(false);
        _setStatus("Extracted " + items.length + " issue(s). Editing unlocked.");
  ...
  function _onPush(ctx) {
    ...
    _setStatus("Pushing to Jira Software\u2026");
    ...
    _api(ctx, "POST", "/jira/" + ctx.sessionId + "/push/" + TYPE + "/", {
      ...
      .then(function (d) {
        ...
        _renderIssues(true);
        _syncFooter();
        _setStatus("Pushed " + count + " issue(s) to Jira Software.");
        if (btn) btn.disabled = false;
      })
      .catch(function (err) {
  ...
  function _bindLeftPaneEvents(ctx) {
    ...
    // Editor delegated events (delete issue + input change)
  ...
  function _renderLeftPane(ctx) {
    return ''
      ...
      + '<h4>Issues <span class="export-modal__count-badge" id="' + PREFIX + '-issue-count">0</span></h4>'
      + '<button type="button" class="btn btn--sm btn--primary" id="' + PREFIX + '-add-issue-btn">Add Issue</button>'
      + '</div>'
  ...
  // ---------------------------------------------------------------------------
  // JiraSoftwareAdapter — the adapter object
  // ---------------------------------------------------------------------------

  var JiraSoftwareAdapter = {
    label: "Jira Software",
  ...
  function _init() {
    ...
    window.ProviderRegistry.register("jira_software", {
      openExportModal: function (ctx) {
        window.ExportModalBase.open(ctx, JiraSoftwareAdapter);
      },
    });
  }
