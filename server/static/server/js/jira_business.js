/**
 * jira_business.js — Jira Business export adapter.
 *
 * Implements the ExportModalBase adapter interface.
 * Registered as provider "jira_business" in window.ProviderRegistry.
 *
...
(function () {
    ...
    _api(ctx, "GET", "/jira/" + ctx.sessionId + "/token-status/" + TYPE + "/")
      .then(function (d) {
        ...
        if (d.valid) {
          ...
          var err = d.error || "Not connected. Configure Jira Business credentials in Project Settings.";
  ...
  function _loadSavedExport(ctx) {
    ...
    _api(ctx, "GET", "/jira/" + ctx.sessionId + "/export/" + encodeURIComponent(ctx.discussionId) + "/" + TYPE + "/")
      .then(function (data) {
        ...
        if (_state.exported) {
          _setStatus("Already exported to Jira Business. Click Extract Items to unlock editing.");
        } else {
  ...
  function _onExtract(ctx) {
    ...
    _api(ctx, "POST", "/jira/" + ctx.sessionId + "/extract/" + encodeURIComponent(ctx.discussionId) + "/" + TYPE + "/")
      .then(function (d) {
        ...
        _state.exported = false;
        _renderIssues(false);
        _setStatus("Extracted " + items.length + " task(s). Editing unlocked.");
  ...
  function _onPush(ctx) {
    ...
    _setStatus("Pushing to Jira Business\u2026");
    ...
    _api(ctx, "POST", "/jira/" + ctx.sessionId + "/push/" + TYPE + "/", {
      ...
      .then(function (d) {
        ...
        _renderIssues(true);
        _syncFooter();
        _setStatus("Pushed " + count + " task(s) to Jira Business.");
        if (btn) btn.disabled = false;
      })
      .catch(function (err) {
  ...
  function _renderLeftPane(ctx) {
    return ''
      ...
      + '<h4>Tasks <span class="export-modal__count-badge" id="' + PREFIX + '-issue-count">0</span></h4>'
      + '<button type="button" class="btn btn--sm btn--primary" id="' + PREFIX + '-add-issue-btn">Add Task</button>'
      + '</div>'
  ...
  // ---------------------------------------------------------------------------
  // JiraBusinessAdapter — the adapter object
  // ---------------------------------------------------------------------------

  var JiraBusinessAdapter = {
    label: "Jira Business",
  ...
  function _init() {
    ...
    window.ProviderRegistry.register("jira_business", {
      openExportModal: function (ctx) {
        window.ExportModalBase.open(ctx, JiraBusinessAdapter);
      },
    });
  }
