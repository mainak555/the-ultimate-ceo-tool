...
57. **Datetime storage standard is mandatory**: all datetime fields in MongoDB must be stored as BSON Date objects (`datetime.now(timezone.utc)`) — never as ISO strings or bare `HH:MM` strings. Use `_utc_now()` from `server/services.py` to produce values. Read paths must coerce BSON datetimes to ISO strings via `_coerce_dt_to_iso()`. Templates must wrap values in `<time class="local-time" data-utc="ISO">` elements; `window.renderLocalTimes()` in `app.js` converts to browser-local display. JS live injections use `new Date().toISOString()` + `window.renderLocalTimes()`. Export pipeline sub-document fields (`discussions[].exports.*.updated_at`, `pushed_at`) are exempt until a dedicated migration. See [`.agents/skills/datetime_storage/SKILL.md`](.agents/skills/datetime_storage/SKILL.md) for the full contract and new-field checklist.
