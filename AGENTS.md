...
61. **Chat attachments are session-scoped by storage key**: all attachment blobs must use key prefix `sessions/{session_id}/...`; no project prefix is allowed. All attachment lookup/bind/read operations must validate `session_id` equality before returning metadata or bytes.
62. **Session delete must clean attachment bytes and metadata**: deleting a chat session must delete all attachment objects under `sessions/{session_id}/` and delete all `chat_attachments` metadata rows in the same operation.
63. **Chat history attachment rendering contract is mandatory**: persisted chat messages must render attachment rows under message markdown content. Image attachments must render thumbnails; non-image attachments render filename links.
64. **HITL markdown + attachment parity is mandatory**: human gate notes must be markdown-rendered in live and persisted history, and the gate input surface must support the same attachment interactions as main chat input (attach button, drag/drop, paste).
65. **Chat attachment implementation skill is required**: before changing chat attachment upload/bind/render/delete behavior, follow [`.agents/skills/chat_attachment_workflow/SKILL.md`](.agents/skills/chat_attachment_workflow/SKILL.md).
66. **Azure attachment auth must use container SAS URL**: attachment storage auth must use `AZURE_STORAGE_CONTAINER_SAS_URL` (container-scoped SAS URL with query token), not account connection strings. SAS tokens should be least-privilege and short-lived where operationally possible.
67. **Single-assistant chat mode is mandatory**: when a project has exactly one assistant agent, the configuration must run in chat mode with Human Gate enabled and Team Setup hidden in the UI. Selector routing is invalid for one assistant. In this mode, runs pause after every assistant turn and continue until the human explicitly selects Stop (no max-iteration completion gate). Empty Continue (no text and no attachments) is invalid in single-assistant mode: the backend must return HTTP 400 before acquiring the Redis lease, and the frontend must keep the Continue button disabled until text is typed or a file is attached.
68. **`AgentMessageTermination` is mandatory for all team termination**: `MaxMessageTermination` from AutoGen must never be used in `agents/team_builder.py`. All termination must use `AgentMessageTermination` (defined in the same module), which counts only messages where `source != "user"` to avoid the off-by-one caused by the task message being included in AutoGen's built-in count. For gated runs, `AgentMessageTermination(n_agents) | ExternalTermination()` is required; the `ExternalTermination` instance must be stashed in `project["_runtime"]["external_termination"]` and registered via `runtime._EXTERNAL_TERMINATIONS[session_id]` so `cancel_team()` can call `.set()` for a graceful stop before the hard `CancellationToken.cancel()` fallback.