...
47. **Langfuse tracing for AutoGen is mandatory**: AutoGen model client and team execution must be observable via the OpenTelemetry exporter wired in `agents/tracing.py`. Wiring is env-gated by `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, and `LANGFUSE_HOST`; missing keys must degrade silently with a `tracing.disabled` info log and no exporter errors. `init_tracing()` is invoked exactly once from `server/apps.py` `ServerConfig.ready()`.
