...
14. `discussions[].content` for human (`role=user`) messages must be set to the raw user-typed task string — never to `text_with_context`. Attachment text (PDF, DOCX, etc.) is a runtime artefact injected into the agent task at run time via `build_attachment_context_block`; it must not appear in MongoDB discussion records.
