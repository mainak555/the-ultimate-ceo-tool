...
Software metadata for the selected destination project also includes an `existing_issues` catalog (`key`, `summary`, `issue_type`) used to power the per-card **Existing Issue** dropdown (`New` + filtered matches).
...
3. If `existing_issue_key` is set on an item, skip create for that row, set `temp_to_key[temp_id] = existing_issue_key`, append a success-style result row, and continue BFS so descendants can attach to the reused Jira key.
...
| `existing_issue_key` | Optional Jira issue key selected in the UI. When present, push reuses that issue (no create call), maps `temp_id` to that key, and continues hierarchy linking for descendants. |
