...
- **`run_stream()` task guard — always inject a non-empty user task**: `views.py` sets `effective_task` to `"Continue."` whenever the resolved task is falsy (empty string or `None`) before calling `team.run_stream()`. This covers two scenarios:
  1. **Empty gate resume (multi-assistant mode)**: When Continue is submitted with no text, `task` is empty and no `UserMessage` is added by AutoGen, leaving each agent's model context ending with its own prior `AssistantMessage`. Claude 3.7+ / Claude 4+ (and future Anthropic models) reject this — they no longer support the "assistant prefill" pattern. The `"Continue."` injection ensures every model call ends with a user turn.
  2. **Attachment-only first run with failed extraction**: If all attachment text extraction fails (e.g. scanned PDF with no text layer), `task_for_agent` collapses to an empty string. Without the guard, `run_stream(task=None)` would produce an empty message list rejected by the API. The `"Continue."` fallback ensures agents still receive a valid user turn.
  
  The synthetic `"Continue."` string is **not persisted** to `discussions[]` and **not shown** in the SSE chat stream (the SSE loop only emits `TextMessage` where `source != "user"`). It is baked into `agent_state` checkpoints, which is correct — the model context accurately records the resume event.
...
- `team_config`:
  - runtime resolves a per-round remote target set and persists it in Redis (`gate_required`) for deterministic quorum checks.
  - leader response is not required in this mode; only targeted remote responders gate handoff.
